...
export const PlansCard: React.FC<props> = ({ children, planType, planPrice }) => {
  return (
    
    <div className="relative w-full max-w-[410px] group">
    <div className="relative bg-white flex flex-col justify-between gap-12 px-[4vw] py-[6vw] h-full rounded-[1vw] border border-transparent cursor-pointer hover:border hover:border-vividIndig z-10">
      <div>
        <h4 className="text-xl mb-2">{planType}</h4>
        <h6 className="relative">
          <span className="absolute top-0">$</span>
          <span className="text-vividIndigo text-5xl font-bold ml-3">{planPrice}</span> Dólares
        </h6>
