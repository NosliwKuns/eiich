...
import { calcTimeDelta, convertToPx, parseTimeDelta } from '@/pages/awards/utils/index';
...
export function FlipClockCountdown(props: FlipClockCountdownProps) {
  ...
  return (
    <div
      ...
      className="flex gap-8 text-red text-5xl"
    >
      {sections.map(([item, label], idx) => {
        return (
          <React.Fragment key={`digit-block-${idx}`}>
            
            <div className="flex gap-2 border border-green-500">
              {/* {showLabels && (
