          ...
          <ul className="flex gap-8 font-semibold text-sm">
            {
              links.map(link => (
                <li key={link.id}>
                  <NavLink to={link.path} className={({ isActive }) => (`${isActive && "text-vividIndigo"} group relative cursor-pointer hover:text-vividIndigo`)}>
                    {({ isActive }) => (
                      <>
                        {link.title}
                        <span className="absolute top-1/2 left-0 w-full h-full"></span>
                        <span className={` ${isActive && "bg-vividIndigo/80"} absolute bottom-[-5px] left-0 w-full h-1 rounded-full group-hover:bg-vividIndigo/80 transition-colors`}></span>
                      </>
                    )}
                  </NavLink>
                </li>
